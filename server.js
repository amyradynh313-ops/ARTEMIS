...
// ---- Generate an image (real generation via Hugging Face Inference API) ----
app.post('/api/generate', requireDevice, async (req, res) => {
  ...
  try {
    const hfRes = await fetch(`https://api-inference.huggingface.co/models/${HF_IMAGE_MODEL}`, {
      method: 'POST',
      headers: {
    ...
    if (!hfRes.ok || !contentType.startsWith('image/')) {
      ...
      console.error('Hugging Face API error:', hfRes.status, detail);
      saveDB(db);
      if (hfRes.status === 503) return res.status(503).json({ error: 'model_loading', detail });
    ...
    if (!sub) d.count += 1;
    saveDB(db);
    res.json({ ok: true, image: dataUrl, used: d.count, subscribed: sub });
  } catch (e) {
    console.error(e);
    saveDB(db);
